...
describe("Categories", () => {
    ...
    it("Selecting a new category should deselect the old one", () => {
        ...
        //Click a category and check that the category is selected
        cy.get('[data-testid="categories-list"]').within(() => {
            cy.get('[data-elementid="jewelery"]').click(); //select
            cy.get('[data-elementid="jewelery"][data-selected="true"]').should("exist"); // category is selected
        });

        //click a different category and check that only the new category is selected
        cy.get('[data-testid="categories-list"]').within(() => {
            cy.get('[data-elementid="electronics"]').click();
            cy.get('[data-selected="true"]').should("have.length", 1);
        });
